...
class GeneralApproach:
    ...
    def __build_lagrangian(self):
        ...
        self.__y_vec = [sympy.Symbol("y" + str(i), real=True) for i in range(len(self.__species_mod_system))]

        # self.__y_vec = [sympy.Symbol("y" + str(i), real=True) for i in range(len(self.__indp_species))]

        self.__x_bar = self.__sympy_reactions + self.__sympy_species + self.__y_vec
        ...
        jac_y_vec = self.__jac_mod_system*sympy.Matrix(self.__y_vec)

        fifth_term = sympy.S.Zero
        for i in jac_y_vec:
            fifth_term += i**2

        sixth_term = sympy.ones(1, len(self.__y_vec))*sympy.Matrix(self.__y_vec)

        self.__lagrangian = self.__det_jac ** 2 + second_term + fifth_term + (sixth_term[0] - numpy.float64(1.0)) ** 2
        ...
        self.__steady_state_func = second_term + (sixth_term[0] - numpy.float64(1.0)) ** 2
    ...
    def __build_objective_function(self):
        ...
        # for i in self.__z_bar:
        #     temp = sympy.diff(self.__lagrangian, i)
        #     self.__obj_func_h += (sympy.simplify(temp))**2

        self.__obj_func_h = self.__lagrangian

        # print(self.__obj_func_h)

        # sys.exit()

        self.__lambda_obj_func_h = self.__lambda_lagrangian #sympy.utilities.lambdify(self.__lagrangian_vars, self.__obj_func_h)

    def __feasible_point_obj_func(self, x, steady_func, x_full, lambda_cons_laws_sympy):
    ...
    def run_optimization(self, bounds=None, iterations=10, seed=0, print_flag=False, feasible_iters = 100, main_iters=1000,
                         confidence_level_flag=False, change_in_rel_error=1e-2, method='dual'):
        ...
        -----------
            ...
            change_in_rel_error: float
                ...
                of :math:`\widetilde{f}`.
            method: string
                Defines the method to be used for the feasible point method and main optimization method. For basinhopping,
                'basin' should be provided. To use dual annealing enter 'dual'.
        Returns
        --------
        params_for_global_min: list of numpy arrays
            A list of numpy arrays that correspond to the decision vectors of the problem.
        ...
        feasible_point_sets, x_full, bounds = self.__feasible_point_method(bounds, iterations, seed, print_flag, confidence_level_flag, feasible_iters, method)
        print("Feasible point method has finished.")
                ...
                with numpy.errstate(divide='ignore', invalid='ignore'):
                    ...
                    if method == 'dual':
                        result = scipy.optimize.dual_annealing(self.__obj_func, bounds=bounds, args=(self.__lambda_obj_func_h, x_full,
                                                               ...
                                                               local_search_options={'method': "Nelder-Mead"}, maxiter=main_iters)
                    elif method == 'basin':
                        result = scipy.optimize.basinhopping(self.__obj_func, feasible_point_sets[i],
                                                             minimizer_kwargs={'method': 'Nelder-Mead',
                                                                               ...
                                                                               'tol': 1e-16},
                                                             niter=main_iters, seed=seed)
                    else:
                        raise Exception("Optimization method provided is not an option. Enter 'basin' or 'dual'.")
    ...
    def __feasible_point_method(self, bounds, iterations, seed, print_flag, confidence_level_flag, feasible_iters, method):

        numpy.random.seed(seed)

        # a temporary bounds for creating random points for the y_vec
        temp_bounds = bounds + [(-100.0, 100.0)]*len(self.__y_vec)

        samples = numpy.random.rand(iterations, len(temp_bounds))

        ranges = numpy.asarray(temp_bounds, dtype=numpy.float64)
        samples = samples * (ranges[:, 1] - ranges[:, 0]) + ranges[:, 0]

        # true bounds for the y_vec
        bounds = bounds + [(-10000.0, 10000.0)] * len(self.__y_vec)
                ...
                with numpy.errstate(divide='ignore', invalid='ignore'):
                    ...
                    if method == 'dual':
                        result = scipy.optimize.dual_annealing(self.__feasible_point_obj_func,
                                                               ...
                                                               x0=samples[i],
                                                               seed=seed,
                                                               local_search_options={'method': "Nelder-Mead"}, maxiter=feasible_iters)
                    elif method == 'basin':
                        result = scipy.optimize.basinhopping(self.__feasible_point_obj_func, samples[i],
                                                             minimizer_kwargs={'method': 'Nelder-Mead',
                                                                               'args': (self.__steady_state_lambda, x_full,
                                                                                        self.__cons_laws_sympy_lamb),
                                                                               'tol': 1e-16},
                                                             niter=feasible_iters, seed=seed)
                    else:
                        raise Exception("Optimization method provided is not an option. Enter 'basin' or 'dual'.")
        ...
        return feasible_point_sets, x_full, bounds
    ...
    def run_direct_simulation(self, params_for_global_min, path):

        import scipy.integrate as itg

        pcp_scan =[numpy.linspace(0.0, 100.0, 50)]*len(params_for_global_min) # [numpy.linspace(90.0, 93.5, 60), numpy.linspace(5.503, 5.508, 60), numpy.linspace(11.3, 11.6, 60)]
    ...
    def __initialize_direct_simulation(self):
        ...
        cons_response_indicies = []
        for i in range(len(self.__replacements)):

            if sympy.Symbol(self.__response, positive=True) in self.__replacements[i][1].atoms():
                cons_response_indicies.append(i)

        cons_response_index = cons_response_indicies[0]

        fwd_scan_vals = []
        for i in range(len(self.__cons_laws_sympy)):
            if i != cons_response_index:
                fwd_scan_vals.append([self.__sympy_species.index(self.__replacements[i][0]), i])
            else:
                fwd_scan_vals.append([spec_index, i])

        rvrs_scan_vals = [[self.__sympy_species.index(self.__replacements[i][0]), i] for i in
                          range(len(self.__cons_laws_sympy))]

        # index to change in forward scan
        fwd_scan_index = [i[0] for i in fwd_scan_vals if i[1] == self.__signal_index][0]  # 2
        # index to change in reverse scan
        rvrs_scan_index = [i[0] for i in rvrs_scan_vals if i[1] == self.__signal_index][0]  # 2
    ...
    def __conduct_fwd_rvrs_scan(self, result_x, fwd_scan_vals, rvrs_scan_vals, pcp_scan, fwd_scan_index,
        ...
        for i in pcp_scan:
            ...
            steady_state = self.__steady_state_finder(initial_species_values, result_x, spec_index, itg)
    ...
    def __steady_state_finder(self, initial_species_values, result_x, spec_index, itg):
        ...
        i = 1
        while abs(out.y[spec_index, -1] - out.y[spec_index, 0]) > 1e-8 and i < 2000:
        ...
        if i >= 2000:
            print("Iterations greater than 2000 occurred in steady state finder.")
        return out.y[:, -1]
    ...
    def get_lambda_conservation_laws(self):

        return self.__cons_laws_sympy_lamb
    ...
    def get_continuation_values(self, params):
        """
        Returns a list of strings that provide the values used by the continuation routine where the index corresponds
        to the params_for_global_min returned by the optimization routine.
        ...
        >>> network = crnt4sbml.CRNT("path/to/SBML_File.xml")
        >>> approach = network.get_general_approach(signal, response)
        >>> approach.get_continuation_values(params)
        """


        values_str = []

        for results_x in params:
