...
class GeneralApproach:
    ...
    def __build_lagrangian(self):
        ...
        self.__y_vec = [sympy.Symbol("y" + str(i), real=True) for i in range(len(self.__indp_system_subs))]

        self.__x_bar = self.__sympy_reactions + self.__sympy_species + self.__y_vec

        self.__x_bar_u = [sympy.Symbol('xu' + str(i), real=True) for i in range(len(self.__x_bar))]

        self.__x_bar_l = [sympy.Symbol('xl' + str(i), real=True) for i in range(len(self.__x_bar))]

        # second_term = sympy.ones(1, self.__N - 1)*self.__modified_ode_system

        # second_term = sympy.ones(1, len(self.__indp_system_subs))*self.__indp_system_subs

        second_term = sympy.S.Zero
        for i in self.__indp_system_subs:
            second_term += i**2

        # tt = [sympy.Max(sympy.S.Zero, self.__x_bar[i] - self.__x_bar_u[i])**2 for i in range(len(self.__x_bar))]
        tt = [sympy.Max(sympy.S.Zero, self.__x_bar[i] - self.__x_bar_u[i]) for i in range(len(self.__x_bar))]
        third_term = sympy.ones(1, len(self.__x_bar))*(sympy.Matrix(tt))

        # tt = [sympy.Max(sympy.S.Zero, self.__x_bar_l[i] - self.__x_bar[i])**2 for i in range(len(self.__x_bar))]
        tt = [sympy.Max(sympy.S.Zero, self.__x_bar_l[i] - self.__x_bar[i]) for i in range(len(self.__x_bar))]
        fourth_term = sympy.ones(1, len(self.__x_bar))*(sympy.Matrix(tt))

        jac_y_vec = self.__jac_mod_system*sympy.Matrix(self.__y_vec)
        # fifth_term = sympy.ones(1, len(self.__y_vec))*jac_y_vec
        ...
        # self.__lagrangian = self.__det_jac**2 + second_term[0]**2 + third_term[0] + fourth_term[0] + fifth_term[0]**2 + (sixth_term[0] - numpy.float64(1.0))**2

        # self.__lagrangian = self.__det_jac ** 2 + abs(second_term[0]) + third_term[0] + fourth_term[0]

        self.__lagrangian = self.__det_jac**2 + second_term + third_term[0] + fourth_term[0] + fifth_term + (sixth_term[0] - numpy.float64(1.0))**2

        # self.__lagrangian = second_term + third_term[0] + fourth_term[0] + fifth_term + (sixth_term[0] - numpy.float64(1.0)) ** 2

        print(self.__lagrangian)

        # self.__lagrangian_vars = self.__x_bar + [sympy.Symbol(self.__signal, real=True)] + self.__x_bar_l + self.__x_bar_u

        self.__lagrangian_vars = self.__x_bar + [sympy.Symbol('C' + str(i + 1), real=True) for i in range(len(self.__cons_laws_sympy))] \
                                 + self.__x_bar_l + self.__x_bar_u

        self.__lambda_lagrangian = sympy.utilities.lambdify(self.__lagrangian_vars, self.__lagrangian)

        # enforcing a steady state
        self.__steady_state_func = second_term + third_term[0] + fourth_term[0] + (sixth_term[0] - numpy.float64(1.0))**2
        # self.__steady_state_func = second_term[0]**2 + third_term[0] + fourth_term[0] + (sixth_term[0] - numpy.float64(1.0))**2
        # self.__steady_state_func = abs(second_term[0]) + third_term[0] + fourth_term[0]
    ...
    def __obj_func(self, x, h_func, x_full, lambda_cons_laws_sympy):
    ...
    def run_optimization(self, bounds=None, iterations=10, seed=0, print_flag=False, feasible_iters = 100, main_iters=1000,
        ...
        feasible_point_sets, x_full, bounds = self.__feasible_point_method(bounds, iterations, seed, print_flag, confidence_level_flag, feasible_iters)
        print("Feasible point method has finished.")
            ...
            for i in range(len(feasible_point_sets)):
                ...
                with numpy.errstate(divide='ignore', invalid='ignore'):
                    ...
                    result = scipy.optimize.dual_annealing(self.__obj_func, bounds=bounds, args=(self.__lambda_obj_func_h, x_full,
                                                                                                 self.__cons_laws_sympy_lamb), x0=feasible_point_sets[i], seed=seed,
    ...
    def __feasible_point_method(self, bounds, iterations, seed, print_flag, confidence_level_flag, feasible_iters):

        numpy.random.seed(seed)

        bounds = bounds + [(-100.0, 100.0)]*len(self.__y_vec)

        samples = numpy.random.rand(iterations, bnds_len)

        ranges = numpy.asarray(bounds, dtype=numpy.float64)
        samples = samples * (ranges[:, 1] - ranges[:, 0]) + ranges[:, 0]

        lower_bounds = numpy.zeros(bnds_len, dtype=numpy.float64)
        upper_bounds = numpy.zeros(bnds_len, dtype=numpy.float64)

        for i in range(bnds_len):
        ...
        x_full[len(bounds) + len(self.__cons_laws_sympy): len(bounds) + len(bounds) + len(self.__cons_laws_sympy)] = lower_bounds
