...
class GeneralApproach:
    ...
    def __feasible_point_obj_func(self, x, boundz, full_set_of_values, reaction_ind, cons_laws_lamb, indp_spec_ind, inputs,
                                  equality_bounds_indices, non_equality_bounds_indices):
        ...
        print("indp_spec_ind")
        print(indp_spec_ind) # self.__sympy_species
        ...
        for i in indp_spec_ind:
            ...
            if i not in equality_bounds_indices:
                inputs[count] = x[i]
                count += 1
            else:
                inputs[count] = boundz[i]
                count += 1
    ...
    def __feasible_point_method(self, bounds, num_constraint_method_iters, seed, print_flag, confidence_level_flag):
        ...
        print(decision_vector_bounds_ind)

        decision_vector_bounds = [bounds[i] for i in decision_vector_bounds_ind]

        print("decision_vector_bounds")
        print(decision_vector_bounds)
        print("")
        # print([all_vars_with_bounds[i] for i in decision_vector_bounds_ind])
        # print("")
        # print(self.__fixed_reactions)

        # testing to see if there are any equalities in bounds
        equality_bounds_indices = []
        for i in range(len(decision_vector_bounds)):
            if not isinstance(decision_vector_bounds[i], numpy.ndarray):
                equality_bounds_indices.append(i)

        non_equality_bounds_indices = [i for i in range(len(decision_vector_bounds)) if
                                       i not in equality_bounds_indices]

        print(equality_bounds_indices)
        print(non_equality_bounds_indices)

        true_bounds = [decision_vector_bounds[i] for i in non_equality_bounds_indices]

        samples = numpy.random.rand(num_constraint_method_iters, len(true_bounds))

        ranges = numpy.asarray(true_bounds, dtype=numpy.float64)
        samples = samples * (ranges[:, 1] - ranges[:, 0]) + ranges[:, 0]

        # import math
        # ranges = numpy.asarray([(math.log10(i[0]), math.log10(i[1])) for i in true_bounds],
        #                        dtype=numpy.float64)
        # samples = samples * (ranges[:, 1] - ranges[:, 0]) + ranges[:, 0]
        ...
        dec_vec_non_equal = [self.__decision_vector[i] for i in range(len(self.__decision_vector)) if i in non_equality_bounds_indices]

        print(self.__decision_vector)
        print(dec_vec_non_equal)

        sys.exit()

        indp_spec_ind_dec = [self.__vars_for_lam_fixed_reactions.index(i) for i in self.__indp_species]
        ...
        if self.__fix_reactions:
            for i in range(num_constraint_method_iters):
                ...
                with numpy.errstate(divide='ignore', invalid='ignore'):
                    ...
                    result = scipy.optimize.dual_annealing(self.__feasible_point_obj_func, bounds=true_bounds,
    ...
    def run_optimization(self, bounds=None, iterations=10, seed=0, print_flag=False, dual_annealing_iters=1000,
        ...
        for i in range(len(bounds)):
            bounds[i] = numpy.float64(bounds[i])
